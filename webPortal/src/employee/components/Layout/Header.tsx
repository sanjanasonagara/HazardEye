...
import { LayoutDashboard, ClipboardList, User, LogOut, Bell, ShieldAlert, RefreshCw, ChevronDown } from 'lucide-react';
...
export const Header: React.FC = () => {
    ...
    // Initialize with local state to avoid pop-in/glitch, then confirm with API
    ...
    const notifications = React.useMemo(() => {
        ...
        // Assuming employees receive alerts
    ...
    const navItems = [
        { path: '/employee', name: 'Dashboard', icon: LayoutDashboard, end: true },
        { path: '/employee/tasks', name: 'My Tasks', icon: ClipboardList },
        { path: '/employee/incidents', name: 'Incidents', icon: ShieldAlert },
        { path: '/employee/resources', name: 'Resources', icon: ClipboardList },
    ];

    const handleRoleSwitch = (role: 'supervisor' | 'employee') => {
        if (role === 'supervisor') {
            navigate('/supervisor');
        } else {
        ...
        <header style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '3rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <ShieldAlert size={28} color="var(--color-hazard, #ef4444)" />
                    <span style={{ fontSize: '1.25rem', fontWeight: 'bold', letterSpacing: '-0.025em' }}>HazardEye Employee</span>
                </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                ...
                {/* Role Switcher - Only for Supervisors viewing as Employee */}
                {/* Check real DB role instead of stale local state */}
                {(realRole?.isSupervisor || realRole?.isAdmin) && (
                    <div style={{ position: 'relative' }}>
                        ...
                        >
                            <span style={{ textTransform: 'capitalize' }}>Employee View</span>
                            <ChevronDown size={14} />
                        </button>
                    ...
                    >
                        <div style={{ textAlign: 'right' }} className="hidden md:block">
                            <p style={{ fontSize: '0.875rem', fontWeight: 600, color: '#e2e8f0', margin: 0 }}>
                                ...
                                {realRole?.isAdmin && <span style={{ marginLeft: '4px', fontSize: '0.7rem', color: '#fbbf24', textTransform: 'uppercase', letterSpacing: '0.5px' }}>(Admin View)</span>}
                            </p>
                            <p style={{ fontSize: '0.75rem', color: '#94a3b8', margin: 0 }}>
                                {realRole?.isAdmin ? 'System Administrator' : realRole?.isSupervisor ? 'Supervisor' : 'Employee'}
                            </p>
                        </div>
                        <div style={{
                            width: '36px',
                        ...
                        <>
                            ...
                            <div style={{
                                ...
                                <div style={{ padding: '4px' }}>
                                    <button
                                        onClick={() => { setShowUserMenu(false); navigate('/employee/profile'); }}
                                ...
                                {notifications.length === 0 ? (
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column' }}>
                                        {notifications.map((note, idx) => (
                                            <div key={idx} style={{ padding: '12px 16px', borderBottom: '1px solid #f8fafc' }}>
